...
data <- read_lines("./data/baby/female.txt")
head(data)
text <- data %>%
  str_to_lower() %>%
  str_c(collapse = "\n") %>%
...
U <- matrix(rnorm(hidden_size*vocab_size)*0.01, nrow=hidden_size) # input
to hidden
W <- matrix(rnorm(hidden_size*hidden_size)*0.01, nrow=hidden_size) # hidden
to hidden
V <- matrix(rnorm(vocab_size*hidden_size)*0.01, nrow=vocab_size) # hidden
to output
...
lossFun <- function(inputs,targets,prev_hidden){
  ...
  return(list("loss"=loss, "dU"=dU, "dW"=dW, "dV"=dV, "dbh"=dbh, "dby"=dby,
              "hs"=hs[length(inputs)-1]))
}
...
while(T){
  ...
  inputs <-
    unlist(sapply(text[p:(p+seq_length)],function(c){which(chars==c)}))
  targets <-
    unlist(sapply(text[(p+1):(p+seq_length+1)],function(c){which(chars==c)}))
